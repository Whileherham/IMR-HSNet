...
class HPNLearner(nn.Module):
    def __init__(self, inch):
        ...
        self.decoder1 = None
        self.decoder2 = None
        # 注意，HSNet代码里，learner需要在这里定义decoder的，但是我们的代码把decoder定义在了hsnet_imr中，所以这里的decoder
        # 其实没用。为了开源代码，必须公司审核，这块不注释掉总是报错“重复率过高”，没办法我只能把这块先注释掉
    ...
    def forward_conv4d(self, hypercorr_pyramid):
    ...
    def forward_decode(self, hypercorr_encoded):
    ...
    def forward(self, hypercorr_pyramid):
        hypercorr_encoded = self.forward_conv4d(hypercorr_pyramid)
        # vgg16: torch.Size([1, 128, 50, 50])
        # r50； torch.Size([1, 128, 50, 50])
        # pdb.set_trace()
        logit_mask = self.forward_decode(hypercorr_encoded)
        return logit_mask

...
class HypercorrSqueezeNetwork_imr(nn.Module):
    ...
    def __init__(self, backbone, use_original_imgsize):
        ...
        if backbone == 'vgg16':
            ...
            # self.conv1024_512 only used in R50, to make sure that the dim is same as that used in VGG
        elif backbone == 'resnet101':
            self.backbone = resnet.resnet101(pretrained=True)
            self.feat_ids = list(range(4, 34))
            self.extract_feats = extract_feat_res
            nbottlenecks = [3, 4, 23, 3]
            # We do not use R101 here
        else:
        ...
        # IMR
        self.state = nn.Parameter(torch.zeros([1,128,50,50])) # 1 128 50 50
    ...
    def forward(self, query_img, support_img, support_cam, query_cam, query_mask=None, support_mask=None, stage=2, w='same'):
        with torch.no_grad():
            ...
            if len(query_feats) == 7:
                isvgg = True # VGG
                q_mid_feat = F.interpolate(query_feats[3] + query_feats[4] + query_feats[5], (50, 50), mode='bilinear',
                                           align_corners=True)
                s_mid_feat = F.interpolate(support_feats[3] + support_feats[4] + support_feats[5], (50, 50),
                                           mode='bilinear', align_corners=True)
            else:
                isvgg = False # R50
                q_mid_feat = F.interpolate(query_feats[4] + query_feats[5] + query_feats[6] + query_feats[7] + query_feats[8] + query_feats[9] , (50, 50),
                                           mode='bilinear', align_corners=True)

                s_mid_feat = F.interpolate(support_feats[4] + support_feats[5] + support_feats[6] + support_feats[7] + support_feats[8] + support_feats[9], (50, 50),
                                           mode='bilinear', align_corners=True)
        ...
        for ss in range(stage):
            ...
            imr_x_query_h = self.convh0(torch.cat([query_cam, after4d_query, q_mid_feat, imr_r_query * state_query], dim=1))
            ...
            imr_x_support_h = self.convh0(torch.cat([support_cam, after4d_support, s_mid_feat, imr_r_support * state_support], dim=1))
            ...
            logit_mask_support = self.res(torch.cat([logit_mask_support, F.interpolate(support_cam, (100,100), mode='bilinear', align_corners=True)], dim=1))
            logit_mask_query = self.res(torch.cat([logit_mask_query, F.interpolate(query_cam, (100,100), mode='bilinear', align_corners=True)], dim=1))

            # loss
            if query_mask != None: # for training
        ...
        if query_mask != None:
            ...
            # test
            if not self.use_original_imgsize:
                logit_mask_query = F.interpolate(logit_mask_query, support_img.size()[2:], mode='bilinear', align_corners=True)
                logit_mask_support = F.interpolate(logit_mask_support, support_img.size()[2:], mode='bilinear', align_corners=True)
    ...
    def predict_mask_0shot(self, batch, nshot, stage):

        # Perform multiple prediction given (nshot) number of different support sets
        logit_mask_agg = 0
        for s_idx in range(nshot):
            logit_mask, logit_mask_s = self(query_img=batch['query_img'],
                                            support_img=batch['query_img'],
                                            support_cam=batch['query_cam'],
                                            query_cam=batch['query_cam'], stage=stage)
            if self.use_original_imgsize:
                org_qry_imsize = tuple([batch['org_query_imsize'][1].item(), batch['org_query_imsize'][0].item()])
                logit_mask = F.interpolate(logit_mask, org_qry_imsize, mode='bilinear', align_corners=True)

    ...
    def forward(self, query_img, support_img, support_cam, query_cam,
        ...
        with torch.no_grad():
            ...
            if len(query_feats) == 7:
                ...
                s_mid_feat = F.interpolate(
                    support_feats[4] + support_feats[5] + support_feats[6] + support_feats[7] + support_feats[8] + support_feats[9],
                    (50, 50), mode='bilinear', align_corners=True)
        ...
        for ss in range(stage):
            ...
            logit_mask_support = self.res(
                torch.cat([logit_mask_support, F.interpolate(support_cam, (100,100), mode='bilinear', align_corners=True)], dim=1))
            logit_mask_query = self.res(
                torch.cat([logit_mask_query, F.interpolate(query_cam, (100,100), mode='bilinear', align_corners=True)], dim=1))
